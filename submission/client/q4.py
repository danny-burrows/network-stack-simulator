...
class EthernetFrame:
    ...
    def to_string_fields(self) -> list[tuple[str, str]]:
        def handle(field_name, value):
            ...
            elif field_name == "preamble":
                return "0b" + "".join([f"{byte:08b}" for byte in value])
    ...
    def get_exam_string(frame: EthernetFrame, note: str = "") -> str:
        def format_value(field_name, value):
            if field_name == "destination_mac" or field_name == "source_mac":
                return f"{value} ({EthernetProtocol.int_to_mac_address(int(value))})"
            elif field_name == "ether_type" and value == "2048":
                return f"{value} (IPv4)"
            return value

        message_fields = "\n".join(
            f"  |- {field_name}: {format_value(field_name, value)}" for field_name, value in frame.to_string_fields()
        )
        ...
        return "\n".join(
            (
                ...
                f"RAW DATA: 0x{frame.to_bytes().hex()}",
                "PROTOCOL: Ethernet",
...
class LinkLayerInterface:
    ...
    def plug_in_and_perform_arp_discovery(self) -> None:
        ...
        self.MAC_ADDRESS = CLIENT_MAC_ADDRESS
        ...
        dest_ip = SERVER_IP_ADDRESS
        dest_mac = SERVER_MAC_ADDRESS
    ...
    def send(self, data: bytes, dest_ip: str) -> None:
        ...
        print(EthernetProtocol.get_exam_string(frame, note="SENDING"))
    ...
    def receive(self) -> bytes:
        ...
        print(EthernetProtocol.get_exam_string(frame, note="RECEIVED"))
        return frame.data
...
class NetworkLayer:
    ...
    def plug_in_and_perform_dhcp_discovery(self) -> None:
        ...
        self.interfaces["eth0"].plug_in_and_perform_arp_discovery()
